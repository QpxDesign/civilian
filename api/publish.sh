...
ssh root@qcloud-1 "cd /root/docker/nginx/sites/civilian/builds && docker load -i civilian-api.tar && docker stop civilian-api-deploy && docker rm civilian-api-deploy &&  docker run --name civilian-api-deploy --restart=always -d --network \"host\" civilian-api"
